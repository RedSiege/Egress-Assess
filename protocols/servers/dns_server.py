        ...
        if not packet.haslayer(DNSRR):
            ...
            for txt in queries: 
                try:
                    ...
                    print "[-] Error base64 decoding: " + txt
    ...
    def customAction(self, packet):
        ...
        if packet.haslayer(DNSQR):
            ...
            if "ENDTHISFILETRANSMISSIONEGRESSASSESS" in dnsqr_strings:
                ...
                if data != False:
                    sys.stdout.write('[*] Transfer Initiated: {}\r'.format(self.file_status))
                    sys.stdout.flush()
                    for d in data:
                        self.extractData(dnsqr_strings, packet, **d)
        return
            ...
            with open(self.loot_path + self.file_name, 'w') as dns_out:
                for dict_key in xrange(1, int(self.file_status) + 1):
                    try:
                        ...
                        print("[-] ERROR: Key {} missing from File Hashtable!".format(dict_key))

        # Clear out the class attributes after file upload is complete
        self.clearAttrs()

    def extractData(self, dnsqr_strings, packet, txt_query="", file_status="", file_data="" ):
        try:
